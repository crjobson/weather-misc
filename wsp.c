...
int read_weather_msg(struct usb_dev_handle *h, char buf[32])
{
  ...
  return usb_bulk_read(h, ENDPOINT_INTERRUPT_ADDRESS, buf, 32, USB_TIMEOUT);
}
...
//
weather_data_t get_history_chunk(struct usb_dev_handle *h, weather_settings_t *ws, unsigned short history_pos, int check)
{
	...
	do
	{
		...
		if (!read_weather_address(h, history_pos, buf))
		{
			...
			if (!check || (check && ((int)buf[0] <= 31)))
			{
			  break;
			}
			fprintf(stderr, "Read interval > 30: %d %d %d 0x%x\n", (int)buf[0], (int)buf[1], (int)buf[4], history_pos);
		}
...
weather_item_t *get_history_item_seconds_delta(weather_settings_t *ws, weather_item_t *history, unsigned int index, int seconds_delta)
{
	...
	if (program_settings.quickrain)
	{
		...
		if ((history[i].timestamp == 0))
		{
			...
			history[i].data = get_history_chunk(devh, ws, new_address, 0);
...
void get_weather_data(struct usb_dev_handle *h)
{
	...
	// we can only get the timestamps by doing it this way.
	{
			...
			history_address -= HISTORY_CHUNK_SIZE, i--, j++)
		{
			...
			history[i].data = get_history_chunk(h, &ws, history_address, (j != 0) ? 1 : 0);
